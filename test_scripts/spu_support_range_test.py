...
if __name__ == "__main__":
    ...
    ln_op = 0 # for Norm, 0: LayerNomm; 1: RMSNorm
    ln_op_str = 'layernorm' if sm_op == 0 else 'rmsnorm'

    # matrix configuration for softmax
    matrix_y_list_sm = [128]
    matrix_x_list_sm = [(4, 60)]  # from segformer_b0 quantization version, with 512 x 480
    if sm_op == 0:
        scales_list_sm = []
        for sm_s_out in range(6, 12+1):
            for sm_s_in in range(0, 7+1):
                for exp_s_out in range(0, 10+1):
                    if sm_s_in + exp_s_out < 6 or sm_s_in + exp_s_out > 10:
                        continue
                    else:
                        scales_list_sm.append((2 ** -sm_s_in, 2 ** -exp_s_out, 2 ** -sm_s_out))
    else:
        scales_list_sm = []
        for sm_s_out in range(6, 12+1):
            for sm_s_in in range(-4, 4+1):
                for exp_s_out in range(6, 12+1):
                        scales_list_sm.append((2 ** -sm_s_in, 2 ** -exp_s_out, 2 ** -sm_s_out))

    # matrix configuration for layernorm
    matrix_y_list_ln = [512]
    matrix_x_list_ln = [(16, 16)]  # from segformer_b0 quantization version, with 512 x 480
    scales_list_ln = [2 ** -9, 2 ** -8, 2 ** -7, 2 ** -6, 2 ** -5, 2 ** -4, 2 ** -3, 2 ** -2, 2 ** -1, 2 ** -0]  # from segformer_b0 quantization version, with 512 x 480
